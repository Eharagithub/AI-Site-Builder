        ...
        {loading ? (
          ...
          <div className="py-10 min-h-[80vh]">
            <div className="flex items-center justify-between mb-10">
              ...
              <button
                onClick={() => navigate("/")}
                className="flex items-center gap-2 px-5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-semibold shadow-lg transition-all text-base"
              >
                <PlusIcon size={20} />
                Create New
              </button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-7">
              {projects.map((project) => (
                <div
                  ...
                  key={project.id}
                  className="project-card bg-gray-800 rounded-xl overflow-hidden shadow-xl hover:shadow-indigo-700/30 border border-gray-700 hover:border-indigo-600 transition-all duration-300 cursor-pointer group flex flex-col relative"
                >
                  {/* Trash Icon Button (appears on hover) */}
                  <button
                    onClick={e => { e.stopPropagation(); deleteProject(project.id); }}
                    className="absolute top-3 right-3 z-20 p-1.5 rounded bg-white/90 text-red-500 text-xl cursor-pointer transition-all opacity-0 group-hover:opacity-100 scale-90 group-hover:scale-100 shadow"
                  ...
                  <div className="p-6 flex-1 flex flex-col justify-between">
                    ...
                    <div onClick={(e) => e.stopPropagation()} className='flex justify-between items-center mt-6'>
                      ...
                      <div className='flex gap-3 text-white text-sm'>
                        ...
                        <button onClick={() => navigate(`/pojects/${project.id}`)} className="px-3 py-1.5 bg-white/10 hover:bg-white/15 rounded-md transition-all">
                          Open
                        </button>
                      </div>
                    </div>
